...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Submit: não salva localStorage nem redireciona aqui.
  // Apenas valida e dispara evento que o script do Firebase irá escutar.
  form.addEventListener('submit', (e) => {
    ...
    // Dispara evento para o script.js (Firebase) processar o envio
    document.dispatchEvent(new CustomEvent('cadastro:submit', { bubbles: true }));
  });

  // Recebe retorno do salvamento (script.js)
  document.addEventListener('cadastro:saved', (ev) => {
    const url = (ev?.detail && ev.detail.redirectTo) || NEXT_URL;
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(ev.detail?.data || {})); } catch {}
    alert('Cadastro de Pessoa Física salvo com sucesso!');
    window.location.href = url;
  });

  document.addEventListener('cadastro:error', (ev) => {
    btnSubmit.disabled = false;
    btnSubmit.textContent = 'Realizar Cadastro';
    const msg = ev?.detail?.message || 'Erro ao salvar cadastro. Tente novamente.';
    alert(msg);
  });
